    ...
    <div class="row">
        ...
        <div class="col-md-9">
            <div class="main-card mb-3 card card-hover-shadow-2x">
                <div class="card-body">
                    <h5 class="card-title">Edit Suami/Istri</h5>
                    <div class="mt-4">
                        <form
                            id="form"
                            method="post"
                            action="{{ 
                            route('fasilitator.suami-istri.update', [
                                    'idSkpd' => $hashidSkpd->encode($skpd->id), 
                                    'id' => $hashidPegawai->encode($pegawai->id_ptt), 
                                    'idSuamiIstri' => $hashid->encode($pasangan->suami_istri_id)
                                ]) 
                            }}"
                        ...
                        >
                            @csrf
                            @method('put')
                            @include('fasilitator.suami_istri._form')
                        </form>
                    </div>
                </div>
            </div>
