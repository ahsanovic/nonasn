...
<script>
    var url = "{{ route('autocomplete-nonaktif') }}";
    $("#search").autocomplete({
        minLength: 3,
    ...
    function aktivasi(niptt) {
        Swal.fire({
            title: 'Yakin akan mengaktifkan pegawai?',
            icon: 'warning',
    ...
    <div class="app-page-title">
        <div class="page-title-wrapper">
            <div class="page-title-heading">
                ...
                <div>
                    Aktivasi Pegawai
                    <div class="page-title-subheading">Pegawai Non ASN Non Aktif</div>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="col-md-12 col-lg-6 col-xl-3">
                <div class="card-shadow-primary card-border mb-3 card">
                    ...
                    <div class="text-center d-block card-footer">
                        <form id="form-aktivasi-{{ $item->niptt }}" method="post" action="{{ route('fasilitator.aktivasi-pegawai', ['niptt' => $item->niptt]) }}">
                            @csrf
                            @method('put')
                        </form>
                        <button onclick="aktivasi('{{ $item->niptt }}')" class="btn btn-sm btn-hover-shine btn-square btn-secondary"><i class="fa fa-power-off"></i> Aktivasi</button>
                    </div>
                </div>
            </div>
        @endforeach
