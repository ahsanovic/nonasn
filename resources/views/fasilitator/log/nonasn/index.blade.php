...
<x-app-layout>
    <x-page-header>
        <div>
            Log Non ASN
            <div class="page-title-subheading">Rekap Log Non ASN</div>
        </div>
    </x-page-header>
    <x-card>
        <h5 class="card-title">Rekap Log Non ASN</h5>
        ...
        <form id="form" method="get" action="{{ url()->current() }}">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-2">
                    <label class="font-weight-bold">Nama / Nomor Induk</label>
                    <input type="text" class="form-control form-control-sm" name="nama" value="{{ request()->input('nama') }}" />
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-2 mt-2">
                    <br />
                    <a href="{{ route('fasilitator.rekap-log-nonasn') }}" class="btn btn-sm btn-hover-shine btn-square btn-danger">Reset</a>
                ...
                <tbody>
                    @forelse ($data as $key => $item)
                        <tr>
                            ...
                            <td>{{ $item->modul }}</td>
                            <td>{{ $item->aksi }}</td>
                            <td>{{ $item->tgl->format('d M Y') . ' / ' . $item->jam }}</td>
                        </tr>
                    @empty
                        <tr>
