...
<script>
var options = {
    series: [{{ $islam }}, {{ $kristen }}, {{ $katolik }}, {{ $hindu }}, {{ $budha }}, {{ $konghucu }}, {{ $lainnya }}],
    chart: {
        width: 480,
        height: 300,
        type: 'donut',
    ...
    labels: ['Islam', 'Kristen/Protestan', 'Katholik', 'Hindu', 'Budha', 'Konghucu', 'Lainnya'],
    responsive: [{
        breakpoint: 480,
        options: {
            chart: {
...
<x-app-layout>
    <x-page-header>
        <div>
            Pegawai Berdasarkan Agama
            <div class="page-title-subheading">Statistik Jumlah Pegawai Berdasarkan Agama</div>
        </div>
    </x-page-header>
    <div class="row">
        <div class="col-md-12 col-lg-6">
            <form id="form" method="get" action="{{ url()->current() }}">
                <div class="position-relative form-group">
                    ...
                    <a href={{ route('stats.agama') }} class="mt-1 btn btn-danger btn-sm btn-square btn-hover-shine btn-clear">Reset</a>
                </div>
            </form>
        </div>
    </div>
