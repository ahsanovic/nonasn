...
<x-app-layout>
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-2">
            <form
                id="form-finish"
                method="post"
                action="{{ route('nonasn.simulasi.cpns.destroy', ['idUjian' => $id_ujian]) }}"
            >
                @csrf
                @method('delete')
            </form>
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 col-lg-12">
                            <form id="form-save" method="post" action="{{ route('nonasn.simulasi.cpns.update', $nomor_sekarang) }}">
                                @method('put')
                                @csrf
                    ...
                    <div class="row">
                        <div class="col md-6 lg-12 mt-4 d-flex">
                                <button 
                                    type="submit" 
                                    {{-- data-url="{{ route('nonasn.simulasi.cpns.update', ['id' => $nomor_sekarang]) }}"  --}}
                                    {{-- data-next="{{ route('nonasn.simulasi.cpns.show', ['id' => $nomor_sekarang + 1]) }}" --}}
                                    class="btn btn-square btn-sm btn-success mr-2" 
                                    id="btn-save">Simpan & Lanjutkan
                                </button>
                            </form>
                            <a 
                                class="btn btn-sm btn-square btn-danger <?php if ($nomor_sekarang == 100) echo 'disabled'; ?>" 
                                {{-- data-url="{{ route('nonasn.simulasi.cpns.show', ['id' => $nomor_sekarang + 1]) }}"  --}}
                                id="btn-skip"
                                href="{{ route('nonasn.simulasi.cpns.show', ['id' => $nomor_sekarang + 1]) }}">Lewati Soal
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
        ...
        <div class="btn-group mr-2 mb-2" role="group" aria-label="First group">
            <a 
                href="{{ route('nonasn.simulasi.cpns.show', ['id' => $nomor_soal]) }}"
